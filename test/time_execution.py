...
simulation(lambda: xml2db(xml_filename="datasets.xml", db_filename="database.db"))()
...
simulation(lambda: xml2db(xml_filename="datasets.xml", db_filename="database.db", unsafe=True))()
...
simulation(lambda: xml2sql(xml_filename="datasets.xml", sql_filename="database.sql"))()
...
simulation(lambda: db2xml(xml_filename="datasets.xml", db_filename="database.db"))()


# -------------------------------------------------------------------
# Save test report
# -------------------------------------------------------------------
with open(f".test_report_{os.path.basename(__file__)}.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(test_report))
